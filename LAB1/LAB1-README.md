...
1. First, we will alter `prefix.AUTH_USER` changelog to `avro-registry` instead of `debezium`.
   ...
   ```
2. Create new `auth_user` table to flatten out `fd.SAMPLE.AUTH_USER` and click `Run`. 
   ```sql
...
5. Next we will do the same for `prefix.USER_TRANSACTION`.
   ...
   ```
6. Create new `user_transaction` table to flatten out `fd.SAMPLE.USER_TRANSACTION` and click `Run`. 
